...
const TableName{{ .MessageName }} = "{{ .MessageName }}"
`

var DynamoDBCreateMessage string = `
func (d *DB) DDBCreate{{ .MessageName }}(ctx context.Context, item *{{ .MessageName }}) (*{{ .MessageName }}, error) {
...
func (d *DB) DDBGet{{ .MessageName }}(ctx context.Context, name string) (*{{ .MessageName }}, error) {
	...
	res, err := d.Client.GetItemWithContext(ctx, input)

	if err != nil {
		return nil, err
	}

	{{ .MessageName | ToLower }} := &{{ .MessageName }}{}

	err = dynamodbattribute.UnmarshalMap(res.Item, activity)

	if err != nil {
		return nil, err
	}

	if {{ .MessageName | ToLower }}.Name == "" {
		return nil, errors.New("Not found")
	}

	return {{ .MessageName | ToLower }}, nil
}`

var DynamoDBGetMessageWithHash = `
func (d *DB) DDBGet{{ .MessageName }}(ctx context.Context, name string) (*{{ .MessageName }}, error) {
